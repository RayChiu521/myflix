...
describe PasswordReset do
  ...
  describe ".token_alive?" do
    ...
    it "returns ture if this token belongs to a user and equals to this user's live_password_token" do
      ...
      expect(PasswordReset.token_alive?(user.live_password_token)).to be_true
    end

    it "returns false if this token not belong to any user" do
      user.generate_password_reset_token
      expect(PasswordReset.token_alive?("test#{user.live_password_token}")).to be_false
    end

    it "returns false if this token belongs to a user but token has expired" do
      password_reset = Fabricate(:password_reset, user: user, expiry_time: Time.now - 1.minute, is_used: false)
      expect(PasswordReset.token_alive?(password_reset.token)).to be_false
    end

    it "returns false if this token belongs to a user but token has been used" do
      password_reset = Fabricate(:password_reset, user: user, expiry_time: Time.now + 1.hour, is_used: true)
      expect(PasswordReset.token_alive?(password_reset.token)).to be_false
    end
  end
  ...
  describe ".update_password!" do
    ...
    context "with alive token" do
      ...
      it "updates user's all password reset records to be used" do
        PasswordReset.update_password!(token, "new_password")
        expect(user.password_resets.where("is_used = ?", false).count).to be_zero
      end

      it "returns true if new password is valid" do
        expect(PasswordReset.update_password!(token, "password")).to be_true
      end

      it "does not update any user's password if new password is blank" do
        old_password_digest = user.password_digest
        PasswordReset.update_password!(token, "")
        expect(user.reload.password_digest).to eq(old_password_digest)
      end

      it "returns false if new password is blank" do
        expect(PasswordReset.update_password!(token, "")).to be_false
      end
    end
    ...
    context "with invalid token" do
      ...
      it "does not updates any user's password" do
        old_password_digest = user.password_digest
        PasswordReset.update_password!(token, "new password")
        expect(user.reload.password_digest).to eq(old_password_digest)
      end

      it "returns false if token not exists" do
        expect(PasswordReset.update_password!("test#{token}", "new password")).to be_false
      end
    end
  end
